...
public class AlgoBinarySearch {
    public static void main(String[] args) {
        ...
        // [162] 寻找峰值
        log.info("FindPeakElement: {}", new FindPeakElement().findPeakElement(new int[] {1,2,3,1}));
        log.info("FindPeakElement: {}", new FindPeakElement().findPeakElement(new int[] {1,2,1,3,5,6,4}));
    }
}
...
class FindPeakElement {
    public int findPeakElement(int[] nums) {
        ...
        return findPeakElement(nums, 0, length - 1);
    }
    ...
    private int findPeakElement(int[] nums, int start, int end) {
        ...
        int rightPeek = findPeakElement(nums, mid, end);
        if (rightPeek > 0) {
            return rightPeek;
        }
        return -1;
