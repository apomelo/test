...
public class AlgoSort {
    public static void main(String[] args) {
        ...
        // [220] 存在重复元素 III
        log.info("ContainsDuplicateIII: {}", new ContainsDuplicateIII().containsNearbyAlmostDuplicate(new int[] {1,2,3,1}, 3, 0));
        log.info("ContainsDuplicateIII: {}", new ContainsDuplicateIII().containsNearbyAlmostDuplicate(new int[] {1,5,9,1,5,9}, 2, 3));
    }
}
...
class ContainsDuplicateIII {
    public boolean containsNearbyAlmostDuplicate(int[] nums, int indexDiff, int valueDiff) {
        ...
        // 使用 TreeSet 存储元素 (本题不会溢出，可能溢出的情况需要用 Long 类型)
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            // 检查是否满足条件
            if ((floor != null && n - floor <= valueDiff) || (ceiling != null && ceiling - n <= valueDiff)) {
                return true;
            }

            // 将当前元素加入 TreeSet
            set.add(n);
            // 检查滑动窗口是否超出范围，移除最旧的元素
            // 窗口内不会出现相等的元素，如果有那么上面判断满足条件时一定满足
            if (i >= indexDiff) { // 等效于 set.size() >= indexDiff + 1
                set.remove(nums[i - indexDiff]);
            }
        }
        // 遍历完成都没有找到符合条件的情况，则返回 false
        return false;
    }
}
